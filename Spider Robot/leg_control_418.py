...
def inverse_kinematics_418(x, z):
    """역기구학 계산 #418"""
    d = math.sqrt(x**2 + z**2)
    if d > L1 + L2:
        return None, None
...
def generate_step_418(step_length=48, step_height=33, points=8):
    """걸음 궤적 생성"""
    trajectory = []
    for i in range(points):
        ...
        a1, a2 = inverse_kinematics_418(L1 + x, z)
        trajectory.append((round(x, 2), round(z, 2), a1, a2))
    return trajectory

if __name__ == "__main__":
    print(f"다리 제어 #418 (L1={L1}mm, L2={L2}mm)")
    traj = generate_step_418()
